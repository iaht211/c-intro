...
{
    struct DNode *a=(*head);
  if(p->next==NULL) return;// neu p = last thi ko doi
   if(p==a){
    (*head)=a->next;
    a->next->prev=NULL;
...
struct DNode *getPrevSong(struct DNode *head, struct DNode *p)
{
    ...
    return NULL;
        ...
        printf("%s\n%s\n%d\n",p->prev->songname,p->prev->singer, 
          p->prev->duration);
          return p->prev;
    }

}
void nhap(char *song,char *singer,int *time)
{
    printf("\nsong name:");
    gets(song);
    printf("\nsinger:");
...
insertLast(&curList,newNode);
newNode = makeNode("afaf","Lionel Richie",345);
insertLast(&curList,newNode);
struct DNode *p=NULL;
...
//removeSong(&curList,p);
/*getPrevSong(curList,p);
...
while(1){
    printf("\n----------MENU-------\n");
    printf("\n1.duyet bai hat theo thu tu");
    printf("\n2.tra ve bai hat ngay truoc /sau");
    printf("\n3.them bai hat vao dau cuoi");
    printf("\n4.thay doi thu tu bai hat");
    printf("\n5.ghi vao file");
    printf("\n6.tim kiem ca si ");
    printf("\nnhap vao lua chon: ");
    scanf("%d",&key);
    switch(key){
        ...
        gets(sName);
        p=searchSongName(curList,sName);
        getNextSong(curList,p);
         printPlayList(curList);
        break;
        case 3:
        printf("\nnhap vao song name:");
        char song[30];
        ...
        int time;
        fflush(stdin);
        nhap(song,singer,&time);
        newNode = makeNode(song,singer,time);
        fflush(stdin);
        gets(sName);
        p=searchSongName(curList,sName);
        insertAfter(p,newNode);
       ...
       fflush(stdin);
gets(namesinger);
listAllSongBySinger(curList,namesinger);
